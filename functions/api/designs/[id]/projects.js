...
export async function onRequestPost({ env, params }) {
  try {
    ...
    const design = await env.DB.prepare("SELECT id, status FROM designs WHERE id = ?").bind(designId).first();
    if (!design || design.status !== "approved") {
      return jsonResponse({ ok: false, error: "This Gallery design is not available." }, 404);
    }

    const instructionObject = await env.DESIGN_IMAGES.get(`instruction-data/${designId}.json`);
    ...
    await env.DB.batch([
      env.DB.prepare(
        `INSERT INTO designs (
          ...
          parts_json, preview_image_key, status, created_at, updated_at
        )
        SELECT ?, title, piece_type, piece_type_name, width, height,
               parts_json, preview_image_key, 'private', ?, ?
