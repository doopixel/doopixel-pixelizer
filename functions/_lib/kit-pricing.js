export const KIT_PRICES_USD_CENTS = Object.freeze({
  "DP-KIT-1X1": 762,
  "DP-KIT-1X2": 1524,
  "DP-KIT-1X3": 2286,
  "DP-KIT-1X4": 3048,
...
export function getKitPrice(artworkWidth, artworkHeight) {
  const width = Number(artworkWidth);
  const height = Number(artworkHeight);
  for (const dimension of [width, height]) {
    if (!Number.isInteger(dimension) || dimension < 16 || dimension > 128 || dimension % 16 !== 0) {
      throw new Error("Artwork dimensions must be 16 to 128 pixels in 16-pixel steps.");
    }
  }

  const baseplateWidth = width / 16;
  const baseplateHeight = height / 16;
  ...
  return { sku, priceCents, currency: "USD" };
}
